package day46_Polymorphism.carTask3;
...
public class Cars {
    ...
    public static void main(String[] args) {
       ...
       int highestMile =  cars[0].miles;
       int lowestMile = cars[0].miles;

        for (Car car : cars) {
            if ((car instanceof Toyota && car.year>=2010 && car.year<=2011)||( car instanceof BMW &&car.year>=1929 && car.year<=2022)||(car instanceof Tesla && car.year>=2015&&car.year<=2016)){
                System.out.println(car.brand+" : "+car.model+" is eligible for recall");
            }
        }
        ...
        for (Car car : cars) {
            ...
            for (int i = 0; i < cars.length; i++) {
                ...
                if(cars[i].miles>highestMile){
                    highestMile = cars[i].miles;
                }

                if(cars[i].miles<lowestMile){
                    lowestMile = cars[i].miles;
                }
            }

            if (highestMile==car.miles){
                System.out.println(car.brand+" "+car.model + " has the highest miles");
            }
            if (lowestMile==car.miles){
                System.out.println(car.brand+" "+car.model + " has the lowest miles");
            }
